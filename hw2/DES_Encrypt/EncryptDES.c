...
static void HexToBit(const char input[], int output[], int bits)//把Hex转换为二进制
{
	int i, j;
	for (j = 0; j<16; j++)
	{
		...
		{
			if (input[j] <= '9') {
                output[3 * (j + 1) - i + j] = ((input[j] - '0') >> i) & 1;
			}
			else if(input[j] <= 'Z') {
				output[3 * (j + 1) - i + j] = ((input[j] - '7') >> i) & 1;
			}
			else {
				output[3 * (j + 1) - i + j] = ((input[j] - 'W') >> i) & 1;
			}
		}
	}
};
static void BitToHex(int input[], char output[], int bits)//把二进制转换为Hex
{
	for (int j = 0; j < 16; j++) {
		int temp = input[j * 4] * 8 + input[j * 4 + 1] * 4 + input[j * 4 + 2] * 2 + input[j * 4 + 3] * 1;
		if(temp > 9){
			output[j] = temp + '7';
		}
		else {
...
static void  DES_Efun(char input[16], char key_in[16], int output[64])
{
	...
	int key[64];
	int l[17][32], r[17][32];
	HexToBit(input,chartobit,16);
	IP(chartobit, Ip, IP_Table);//正确，IP初始置换！
	HexToBit(key_in,key,16);
...
int main(int argc,char* argv[])
{
	...
	DES_Efun(plaintext+2, key+2, output);
	BitToHex(output,cipertext,16);
        printf("0x");
	for (int i = 0; i<16; i++)
	{
